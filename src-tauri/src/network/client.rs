...
impl TcpClient {
    ...
    pub async fn connect(&mut self) -> Result<(), String> {
        ...
        let buffer_str =
            std::str::from_utf8(&chunk).map_err(|_| "Invalid UTF-8 received from main node")?;
    ...
    pub async fn send_message(&mut self, chat_id: &str, dst_id_hexs: &str, message_string: &str) {
        {
            ...
            let ss = crate::crypto::keys::ratchet_forward(&"send_root_secret", &chat_id)
                .await
                .unwrap();
            let encrypted_packet = crate::network::packet::create_send_message_packet(
                dst_id_hexs,
                message_string,
                &ss.to_vec(),
                &nss,
    ...
    pub async fn write_enc(&mut self, raw_packet: &Vec<u8>) {
        let nss = self.get_node_shared_secret().await;
